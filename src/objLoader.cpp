...
// std::unordered_map<int, uint32_t> vertexCMap;
    // struct VertexKey
    // {
    //     int v;
    // };
...
ModelData loadOBJ(const std::string& path)
{
    ...
    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, path.c_str(), "assets/", true)) {
    ...
    for(const auto& mat : materials) {
        ...
        m.emissive = glm::vec3(mat.emission[0], mat.emission[1], mat.emission[2]);
        m.shininess = mat.shininess;
        modelData.materials.push_back(m);
    }
